...
async function checkNewAuthors(listCalibreAuthors) {
  ...
  authorsToCreate = _.uniqWith(tempAuthorsToCreate, _.isEqual)

  dbg(chalk.green("CREATE AUTHORS DUPS:\n"), tempAuthorsToCreate, "\n");
...
function addGender(data, list) {
  for (const name of list) {
    ...
    if (first != null) {
      const searchObj = data.find((row) => row.name == first);
      author.gender = typeof searchObj?.gender == "undefined" ? "x" : searchObj.gender.replace("?", "").toLowerCase();
    }
  }
