...
export async function addNewBooks() {
  ...
  spinner.succeed(chalk.yellow(`All done! ${books.length} books and ${authorsToCreate.length} authors created.`));
    ...
    const pages = await Promise.all(
      authorsToCreate.map((author) => {
        ...
        const req = {
          ...
          icon: {
            type: "external",
            external: { url: `${process.env.IMAGES_WEB}letter-${authorIconSuffix}.svg` },
          },    
          properties: {
            Name: {
...
/**
 * That's running. Now I need to:
 * 1. Format the author name
 * 2. Add a field for the full name
 * 3. Create and copy the cover to icon and hero
 * 4. Check the gender of the author
 * 5. Create an svg with the initials: pink for girls, blue for boys? :)
 */
...
function addGender(data, list) {
  for (const author of list ) {
    ...
    author.gender = typeof searchObj?.gender == 'undefined' ? "X" : searchObj.gender;
  }
